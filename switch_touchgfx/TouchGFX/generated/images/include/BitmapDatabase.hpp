// Generated by imageconverter. Please, do not edit!

#ifndef BITMAPDATABASE_HPP
#define BITMAPDATABASE_HPP

#include <touchgfx/hal/Types.hpp>
#include <touchgfx/Bitmap.hpp>

const uint16_t BITMAP_TRANSPARENCYSMALL_ID = 0; // Size: 355x239 pixels
const uint16_t BITMAP_BLUE_BUTTONS_ROUND_ICON_BUTTON_ID = 1; // Size: 60x60 pixels
const uint16_t BITMAP_BLUE_BUTTONS_ROUND_ICON_BUTTON_PRESSED_ID = 2; // Size: 60x60 pixels
const uint16_t BITMAP_MAIN_SCREEN_ID = 3; // Size: 480x272 pixels
const uint16_t BITMAP_WINDOW_SETTINGS_ID = 4; // Size: 400x261 pixels

namespace BitmapDatabase
{
#ifndef NO_USING_NAMESPACE_TOUCHGFX
using namespace touchgfx;
#endif

class BitmapData;
const touchgfx::Bitmap::BitmapData* getInstance();
uint16_t getInstanceSize();
}

#endif
